        ...
        foreach (MediaType::loadMultiple() as $type) {
            ...
            if (empty($source_field_definition)) {
                $requirements['media_missing_source_field_' . $type->id()] = [
                  'title' => $this->t('Media'),
                  'description' => $this->t(
                      'The source field definition for the %type media type is missing.',
                      [
                      '%type' => $type->label(),
            ]
                  ),
            ...
            if ($this->moduleHandler->moduleExists('field_ui') && $this->currentUser->hasPermission('administer media display')) {
                ...
                $action_item = new TranslatableMarkup(
                    'If you would like to change this, <a href=":display">add an image style to the %field_name field</a>.',
                    [
            ...
            $requirements['media_default_image_style_' . $type->id()] = [
              ...
              'description' => new TranslatableMarkup(
                  'The default display for the %type media type is not currently using an image style on the %field_name field. Not using an image style can lead to much larger file downloads. @action_item',
                  [
                  '%field_name' => $source_field_definition->label(),
