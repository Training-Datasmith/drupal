    ...
    public function assertFilteredString(FilterInterface $filter, array $tests): void
    {
        foreach ($tests as $source => $tasks) {
            ...
            foreach ($tasks as $value => $is_expected) {
                if ($is_expected) {
                    $this->assertStringContainsString($value, $result, sprintf(
                        '%s: %s found. Filtered result: %s.',
                        var_export($source, true),
                        var_export($value, true),
                        var_export($result, true),
                    ));
                } else {
                    $this->assertStringNotContainsString($value, $result, sprintf(
                        '%s: %s not found. Filtered result: %s.',
                        var_export($source, true),
                        var_export($value, true),
                        var_export($result, true),
                    ));
