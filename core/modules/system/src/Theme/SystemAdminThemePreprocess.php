...
class SystemAdminThemePreprocess {
  ...
  public function preprocessSystemModulesDetails(array &$variables): void {
    ...
    foreach (Element::children($form) as $key) {
      ...
      $module['machine_name'] = $this->renderer->render($machine_name_render);
      ...
      if (!empty($module['#requires'])) {
        ...
        $module['requires'] = $this->renderer->render($requires);
      }
      if (!empty($module['#required_by'])) {
        ...
        $module['required_by'] = $this->renderer->render($required_by);
      }
    ...
    foreach ($variables['theme_group_titles'] as $state => $title) {
      ...
      foreach ($theme_groups[$state] as $theme) {
        ...
        if ($theme->screenshot) {
          ...
          $current_theme['screenshot'] = [
            '#theme' => 'image',
            '#uri' => $this->moduleExtensionList->getPath('system') . '/images/no_screenshot.png',
        ...
        if (!empty($theme->info['core_incompatible'])) {
          $current_theme['incompatible'] = $this->t("This theme is not compatible with Drupal @core_version. Check that the .info.yml file contains a compatible 'core' or 'core_version_requirement' value.", ['@core_version' => \Drupal::VERSION]);
        }
        ...
        elseif (!empty($theme->incompatible_engine)) {
          $current_theme['incompatible'] = $this->t('This theme requires the theme engine @theme_engine to operate correctly.', ['@theme_engine' => $theme->info['engine']]);
        }
  ...
  public function preprocessSystemSecurityAdvisoriesFetchErrorMessage(array &$variables): void {
    $variables['error_message'] = [
      ...
      'items' => [
        '#theme' => 'item_list',
        '#items' => [
          'documentation_link' => $this->t('See <a href=":url">Troubleshooting the advisory feed</a> for possible causes and resolutions.', [':url' => 'https://www.drupal.org/docs/updating-drupal/responding-to-critical-security-update-advisories#s-troubleshooting-the-advisory-feed']),
        ],
      ],
    ];
