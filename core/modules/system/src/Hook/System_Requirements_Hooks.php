...
class SystemRequirementsHooks
{
    ...
    public function checkRequirements(string $phase): array
    {
        ...
        if ($phase == 'runtime') {
            ...
            if (!empty($experimental_modules)) {
                $requirements['experimental_modules'] = [
                  ...
                  'value' => $this->t(
                      'Experimental modules found: %module_list. <a href=":url">Experimental modules</a> are provided for testing purposes only. Use at your own risk.',
                      [
        ...
        elseif ($phase === 'runtime' && version_compare($phpversion, \Drupal::RECOMMENDED_PHP) < 0) {
            $requirements['php']['description'] = $this->t(
                'It is recommended to upgrade to PHP version %recommended or higher for the best ongoing support.  See <a href="http://php.net/supported-versions.php">PHP\'s version support documentation</a> and the <a href=":php_requirements">Drupal PHP requirements</a> page for more information.',
                [
                '%recommended' => \Drupal::RECOMMENDED_PHP,
                ':php_requirements' => 'https://www.drupal.org/docs/system-requirements/php-requirements',
        ]
        ...
        if (empty($config_sync_directory)) {
            ...
            $requirements['config sync directory'] = [
              ...
              'description' => $this->t(
                  'Your %file file must define the %setting setting as a string containing the directory in which configuration files can be found.',
                  [
            ...
            if (str_contains($autoload, 'src/Driver/Database/')) {
                ...
                if (!in_array('enable_provider_database_driver', array_keys($pending_updates['system']['pending'] ?? []), true)) {
                    ...
                    $requirements['database_driver_provided_by_module'] = [
                      ...
                      'description' => $this->t(
                          'The current database driver is provided by the module: %module. The module is currently not installed. You should immediately <a href=":install">install</a> the module.',
                          [
                          '%module' => $provider,
