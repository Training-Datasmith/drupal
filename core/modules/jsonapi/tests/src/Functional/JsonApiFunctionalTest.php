...
class JsonApiFunctionalTest extends JsonApiFunctionalTestBase
{
    ...
    public function testWrite(): void
    {
        ...
        $this->assertEquals(403, $response->getStatusCode());
        $this->assertEquals(
            "The current user is not allowed to PATCH the selected field (status). The following permissions are required: 'administer node published status' OR 'administer nodes'.",
        ...
        $this->assertEquals(422, $response->getStatusCode());
        $this->assertEquals(
            'The attribute field_that_does_not_exist does not exist on the node--article resource type.',
            $updated_response['errors']['0']['detail']
        );
