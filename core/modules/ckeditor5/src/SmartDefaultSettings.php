        ...
        if ($has_html_restrictions) {
            ...
            if (!empty($plugins_enabled) || !$source_editing_additions->allowsNothing()) {
                ...
                if (!$source_editing_additions->allowsNothing()) {
                    $source_editing_info = $help_enabled ?
                      $this->t(
                          'Added these tags/attributes to the Source Editing Plugin\'s <a target="_blank" href=":source_edit_url">Manually editable HTML tags</a> setting: @tag_list',
                          [
                ...
                $end = $can_access_dblog ?
                  $this->t(
                      'Additional details are available <a target="_blank" href=":dblog_url">in your logs</a>.',
                      [
            ...
            if (!$missing_fundamental_tags->allowsNothing() || !empty($attributes_to_tag) || !empty($added_tags)) {
                ...
                if ($help_enabled && !$missing_fundamental_tags->allowsNothing()) {
                    $fundamental_tags = $this->formatPlural(
                        count($missing_fundamental_tags->toCKEditor5ElementsArray()),
                ...
                $messages[MessengerInterface::TYPE_WARNING][] = new FormattableMarkup(
                    '@beginning @added_elements_begin @fundamental_tags @added_elements_tags @added_elements_attributes @end',
                    [
                    '@beginning' => $beginning,
