...
class FileImageDimensionsConstraintValidator extends BaseFileConstraintValidator implements ContainerInjectionInterface
{
    ...
    public function validate(mixed $value, Constraint $constraint): void
    {
        ...
        if ($maxDimensions) {
            ...
            [$width, $height] = explode('x', (string) $maxDimensions);
        ...
        if ($minDimensions) {
            ...
            [$width, $height] = explode('x', (string) $minDimensions);
            if ($image->getWidth() < $width || $image->getHeight() < $height) {
                if ($scaling) {
                    $this->context->addViolation(
                        $constraint->messageResizedImageTooSmall,
                        [
                ...
                $this->context->addViolation(
                    $constraint->messageImageTooSmall,
                    [
                    '%dimensions' => $minDimensions,
                    '%width' => $image->getWidth(),
                    '%height' => $image->getHeight(),
