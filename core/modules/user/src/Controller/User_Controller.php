    ...
    public function resetPass(Request $request, $uid, $timestamp, $hash)
    {
        ...
        if ($account->isAuthenticated()) {
            ...
            if ($reset_link_user && $this->validatePathParameters($reset_link_user, $timestamp, $hash)) {
                $this->messenger()
                  ->addWarning($this->t(
                      'Another user (%other_user) is already logged into the site on this computer, but you tried to use a one-time link for user %resetting_user. <a href=":logout">Log out</a> and try using the link again.',
                      [
