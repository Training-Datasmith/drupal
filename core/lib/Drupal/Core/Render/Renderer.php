...
class Renderer implements RendererInterface
{
     ...
     * {@inheritdoc}
     */
    public function renderInIsolation(&$elements): \Drupal\Component\Render\MarkupInterface|string
    {
        $context = new RenderContext();
        return Markup::create($this->executeInRenderContext($context, function () use (&$elements, $context): \Drupal\Component\Render\MarkupInterface|string {
     ...
     * {@inheritdoc}
     */
    public function render(array &$elements): string|\Drupal\Component\Render\MarkupInterface
    {
    ...
    protected function doRender(array &$elements, RenderContext $context): string|MarkupInterface
    {
        ...
        if (isset($elements['#lazy_builder'])) {
            ...
            assert(count($elements['#lazy_builder'][1]) === count(array_filter($elements['#lazy_builder'][1], fn ($v) => is_null($v) || is_scalar($v))), "A #lazy_builder callback's context may only contain scalar values or NULL.");
     ...
     * @return $this
     */
    protected function setCurrentRenderContext(?RenderContext $context = null): static
    {
        $request = $this->requestStack->getCurrentRequest();
     ...
     */
    protected function replacePlaceholders(array &$elements): bool
    {
        ...
        foreach ($elements['#attached']['placeholders'] as $placeholder => $placeholder_element) {
            if (isset($placeholder_element['#lazy_builder']) && $placeholder_element['#lazy_builder'][0] === 'Drupal\Core\Render\Element\StatusMessages::renderMessages') {
                ...
                // Get the render array for the given placeholder.
                $fibers[$placeholder] = new \Fiber(fn () => [$this->doRenderPlaceholder($placeholder_element), $placeholder_element]);
            }
        }
        $iterations = 0;
     ...
     *   again.
     */
    protected function xssFilterAdminIfUnsafe($string): \Drupal\Component\Render\MarkupInterface|string
    {
     ...
     */
    protected function doCallback(string $callback_type, callable|array|string $callback, array $args): mixed
    {
     ...
     */
    protected function addDebugOutput(array $elements, bool $is_cache_hit, array $pre_bubbling_elements = [], float $render_time = 0): array
    {
