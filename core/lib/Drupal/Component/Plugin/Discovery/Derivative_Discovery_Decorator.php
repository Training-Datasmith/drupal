...
use Drupal\Component\Plugin\Definition\Derivable_Plugin_Definition_Interface;
use Drupal\Component\Plugin\Exception\Invalid_Deriver_Exception;
/**
 ...
 */
class Derivative_Discovery_Decorator implements Cached_Discovery_Interface
{
    use Discovery_Trait;
    /**
     * Plugin derivers.
     ...
     *   decorated.
     */
    public function __construct(protected \Drupal\Component\Plugin\Discovery\Discovery_Interface $decorated)
    {
    }
    /**
     * {@inheritdoc}
    ...
    public function get_definition($plugin_id, $exception_on_invalid = true)
    {
        ...
        // $base_plugin_id.
        $plugin_definition = $this->decorated->get_definition($plugin_id, false);
        [$base_plugin_id, $derivative_id] = $this->decode_plugin_id($plugin_id);
        $base_plugin_definition = $this->decorated->get_definition($base_plugin_id, $exception_on_invalid);
        if ($base_plugin_definition) {
            $deriver = $this->get_deriver($base_plugin_id, $base_plugin_definition);
            if ($deriver) {
                $derivative_plugin_definition = $deriver->get_derivative_definition($derivative_id, $base_plugin_definition);
                ...
                if ($derivative_id && isset($plugin_definition)) {
                    $plugin_definition = $this->merge_derivative_definition($plugin_definition, $derivative_plugin_definition);
                } else {
    ...
    {
        $plugin_definitions = $this->decorated->get_definitions();
        return $this->get_derivatives($plugin_definitions);
    }
    /**
     ...
     * @return mixed[]
     */
    protected function get_derivatives(array $base_plugin_definitions): array
    {
        $plugin_definitions = [];
        foreach ($base_plugin_definitions as $base_plugin_id => $plugin_definition) {
            $deriver = $this->get_deriver($base_plugin_id, $plugin_definition);
            if ($deriver) {
                $derivative_definitions = $deriver->get_derivative_definitions($plugin_definition);
                foreach ($derivative_definitions as $derivative_id => $derivative_definition) {
                    $plugin_id = $this->encode_plugin_id($base_plugin_id, $derivative_id);
                    ...
                    if ($derivative_id && isset($base_plugin_definitions[$plugin_id])) {
                        $derivative_definition = $this->merge_derivative_definition($base_plugin_definitions[$plugin_id], $derivative_definition);
                    }
     ...
     */
    protected function decode_plugin_id($plugin_id): array
    {
     ...
     */
    protected function encode_plugin_id($base_plugin_id, $derivative_id)
    {
        if ($derivative_id) {
     ...
     *   does not implement \Drupal\Component\Plugin\Derivative\DeriverInterface.
     */
    protected function get_deriver($base_plugin_id, $base_definition)
    {
        if (!isset($this->derivers[$base_plugin_id])) {
            $this->derivers[$base_plugin_id] = false;
            $class = $this->get_deriver_class($base_definition);
            if ($class) {
     ...
     */
    protected function get_deriver_class($base_definition): string|array|int|float|false|null
    {
        $class = null;
        $id = null;
        if ($base_definition instanceof Derivable_Plugin_Definition_Interface) {
            $class = $base_definition->get_deriver();
        ...
        if ($class) {
            if (!class_exists($class)) {
                throw new Invalid_Deriver_Exception(sprintf('Plugin (%s) deriver "%s" does not exist.', $id, $class));
            }
            if (!is_subclass_of($class, \Drupal\Component\Plugin\Derivative\Deriver_Interface::class)) {
                throw new Invalid_Deriver_Exception(sprintf('Plugin (%s) deriver "%s" must implement \Drupal\Component\Plugin\Derivative\DeriverInterface.', $id, $class));
            }
        }
        return $class;
    }
     ...
     */
    protected function merge_derivative_definition($base_plugin_definition, $derivative_definition)
    {
     ...
     * {@inheritdoc}
     */
    public function use_caches($use_caches = false): void
    {
        if (!$use_caches) {
            $this->clear_cached_definitions();
        }
    }
    /**
