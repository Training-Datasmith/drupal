 ...
 */
class Po_Header implements \Stringable
{
    /**
     * Formula for the plural form.
     */
    protected string $plural_forms;
    /**
    ...
    )
    {
        ...
        $this->po_date = @date('Y-m-d H:iO');
        $this->plural_forms = 'nplurals=2; plural=(n > 1);';
    }
    /**
     ...
     *   Human readable language name.
     */
    public function set_language_name($language_name): void
    {
     ...
     */
    public function set_project_name($project_name): void
    {
     ...
     */
    public function set_from_string($header): void
    {
        // Get an array of all header values for processing.
        $values = $this->parse_header($header);
        ...
        if (!empty($values['Plural-Forms'])) {
            $this->plural_forms = $values['Plural-Forms'];
        }
    }
    /**
    ...
    {
        $output = '';
        $is_template = empty($this->language_name);
        $output .= '# ' . ($is_template ? 'LANGUAGE' : $this->language_name) . ' translation of ' . ($is_template ? 'PROJECT' : $this->project_name) . "\n";
        ...
        $output .= '"POT-Creation-Date: ' . $this->po_date . "\\n\"\n";
        $output .= '"PO-Revision-Date: ' . $this->po_date . "\\n\"\n";
        $output .= "\"Last-Translator: NAME <EMAIL@ADDRESS>\\n\"\n";
        $output .= "\"Language-Team: LANGUAGE <EMAIL@ADDRESS>\\n\"\n";
        $output .= "\"MIME-Version: 1.0\\n\"\n";
        $output .= "\"Content-Type: text/plain; charset=utf-8\\n\"\n";
        $output .= "\"Content-Transfer-Encoding: 8bit\\n\"\n";
        $output .= '"Plural-Forms: ' . $this->plural_forms . "\\n\"\n";
        return $output . "\n";
    }
    /**
     ...
     * @throws \Exception
     */
    public function parse_plural_forms($plural_forms): false|array
    {
        $plurals = [];
        ...
        $element_stack = $this->parse_arithmetic($plural);
        if ($element_stack !== false) {
            for ($i = 0; $i <= 199; $i++) {
                $plurals[$i] = $this->evaluate_plural($element_stack, $i);
            }
            $default = $plurals[$i - 1];
     ...
     */
    private function parse_header($header): array
    {
        $header_parsed = [];
     ...
     *   could not be parsed.
     */
    private function parse_arithmetic(string $string)
    {
        // Operator precedence table.
        $precedence = ['(' => -1, ')' => -1, '?' => 1, ':' => 1, '||' => 3, '&&' => 4, '==' => 5, '!=' => 5, '<' => 6, '>' => 6, '<=' => 6, '>=' => 6, '+' => 7, '-' => 7, '*' => 8, '/' => 8, '%' => 8];
        // Right associativity.
        $right_associativity = ['?' => 1, ':' => 1];
        $tokens = $this->tokenize_formula($string);
     ...
     */
    private function tokenize_formula($formula): array
    {
     ...
     * @throws \Exception
     */
    protected function evaluate_plural(array $element_stack, $n): int
    {
