 ...
 */
class Po_Item implements \Stringable
{
    /**
     ...
     *   plurals.
     */
    public function set_translation($translation): void
    {
     ...
     */
    public function set_plural($plural): void
    {
        $this->plural = $plural;
    }
    /**
     ...
     */
    public function set_from_array(array $values = []): void
    {
        if (isset($values['context'])) {
            $this->set_context($values['context']);
        }
        if (isset($values['source'])) {
            $this->set_source($values['source']);
        }
        if (isset($values['translation'])) {
            $this->set_translation($values['translation']);
        }
        if (isset($values['comment'])) {
            $this->set_comment($values['comment']);
        }
        if (isset($this->source) && str_contains($this->source, self::DELIMITER)) {
            $this->set_source(explode(self::DELIMITER, $this->source));
            $this->set_translation(explode(self::DELIMITER, $this->translation ?? ''));
            $this->set_plural(count($this->source) > 1);
        }
    }
    /**
    ...
    {
        return (string) $this->format_item();
    }
    /**
     * Format the POItem as a string.
     */
    private function format_item(): string
    {
        $output = '';
        // Format string context.
        if (!empty($this->context)) {
            $output .= 'msgctxt ' . $this->format_string($this->context);
        }
        // Format translation.
        if ($this->plural) {
            $output .= $this->format_plural();
        } else {
            $output .= $this->format_singular();
        }
     ...
     */
    private function format_plural(): string
    {
        $output = '';
        // Format source strings.
        $output .= 'msgid ' . $this->format_string($this->source[0]);
        $output .= 'msgid_plural ' . $this->format_string($this->source[1]);
        foreach ($this->translation as $i => $trans) {
            if (isset($this->translation[$i])) {
                $output .= 'msgstr[' . $i . '] ' . $this->format_string($trans);
            } else {
                $output .= 'msgstr[' . $i . '] ""' . "\n";
            }
        }
     ...
     */
    private function format_singular(): string
    {
        $output = '';
        $output .= 'msgid ' . $this->format_string($this->source);
        return $output . ('msgstr ' . isset($this->translation) ? $this->format_string($this->translation) : '""' . "\n");
    }
    /**
    ...
    private function format_string($string): string
    {
        ...
        $string = addcslashes((string) $string, "\x00..\x1f\\\"");
